...
class EmployeeRepository {
  ...
  Future<Resource<Employee>> addEmployee(
      AddEmployeeRequest addEmployeeRequest) async {
    ...
    final resource = NetworkBoundResource<Employee, Employee>(
      createCall: () =>
          APIService.addEmployee(companyId, addEmployeeRequest, _cancelToken),
      parsedData: (json) => Employee.fromJson(json),
      saveCallResult: (data) async => await employeeDao
          .insertEmployee(data.copyWith(employeeConfirm: Value("W"))),
    );
  ...
  Future<Resource<dynamic>> editEmployee(
    ...
    final resource = NetworkBoundResource<dynamic, dynamic>(
      createCall: () => APIService.editEmployee(
          employeeId, companyId, addEmployeeRequest, _cancelToken),
      parsedData: (json) => Employee.fromJson(json),
  ...
  Future<Resource<ListResponse<Employee>>> getEmployeeList() async {
    ...
    final companyId = user.company?.id ?? -1;
    final resource =
        NetworkBoundResource<ListResponse<Employee>, ListResponse<Employee>>(
  ...
  Future<Resource<dynamic>> deleteEmployee(int employeeId) async {
    ...
    final resource = NetworkBoundResource<dynamic, dynamic>(
      createCall: () =>
          APIService.deleteEmployee(employeeId, companyId, _cancelToken),
